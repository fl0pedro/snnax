...
# Last Modified : Mon 17 Feb 2025 02:42:50 PM CET
#
...
class SNNMLP(eqx.Module):
    ...
    def get_cumsum(self,x,key, seqlen=None):
        state = self.cell.init_state(in_shape=[x[0,:].shape], shapes=self.shapes, key=key)
...
def make_layers(in_channels, hid_channels, out_channels, key, neuron_model, use_bias=True, num_hid_layers=2, alpha=0.95, beta=.85, norm=False):
    surr = sr(beta = 10.0)
    shapes = []
    layers = [snn.Flatten()]
    ...
    for i in range(num_hid_layers):
        ...
        if not norm:
            ...
            _ff_norm = eqx.nn.LayerNorm(shape=(_out_s,), elementwise_affine=False, eps=1e-4)
            _ff = eqx.nn.Sequential([eqx.nn.Linear(in_channels, _out_s, key=init_key, use_bias=use_bias), _ff_norm])
            _rec_norm = eqx.nn.LayerNorm(shape=(_out_s,), elementwise_affine=False, eps=1e-4)
            _rec = eqx.nn.Sequential([LinearNonDiag(_out_s, _out_s, key=init_key, use_bias=False),_rec_norm])
            m.append(snn.composed.Parallel([_ff,_rec]))
    ...
    init_key, key = jrandom.split(key,2)
    layers.append(eqx.nn.Linear(hid_channels, out_channels, key=key, use_bias=use_bias))
    shapes.append(hid_channels)
